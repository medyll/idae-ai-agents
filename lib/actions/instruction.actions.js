...
export async function addInstructionAction(id, { force = false, dest = DEST, merge = true } = {}) {
  ...
  if (regData.type !== 'instruction') {
    console.error(chalk.red(`✗  "${id}" is not an instruction (type: ${regData.type}).`));
    process.exit(1);
  }
  ...
  if (installed && !force) {
    ...
    console.log(chalk.blue(`📋  Installing instruction: ${id} v${regData.version}`));
...
async function _rebuildMergedFile(dest) {
  ...
  const merged = [
    `<!-- Auto-generated by @medyll/ia-agents — do not edit manually -->`,
  ...
  console.log(chalk.gray(`   ↳ Rebuilt ${COPILOT_FILE} (${globals.length} global instruction${globals.length > 1 ? 's' : ''})`));
}
